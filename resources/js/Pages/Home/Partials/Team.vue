...
<template>
    <MainModal :open="open" :name="'Team'">
        <section class="m-1 mx-3 border p-3 rounded-lg  w-1/3 h-full">
            ...
            <ul class="grid grid-cols-3 grid-rows-2 gap-4 wrap ">
        ...
        <section class="border p-3 rounded-lg overflow-y-scroll w-1/3 h-full m-1 mx-3">
            ...
            <ul class="grid grid-cols-4 gap-4 wrap ">
                ...
                <template v-if="display == 'item'" v-for="n in nbSlotB">
                    <li @click="pick = n-1" class="h-26 w-26 hover:bg-violet-300 bg-white border border-gray-300 rounded-lg shadow  active:bg-violet-800 focus:outline-none focus:ring focus:ring-violet-500 " :class="{'bg-violet-300': pick==n-1}">
